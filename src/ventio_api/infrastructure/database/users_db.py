...
class UserDatabase(BaseDatabase[User]):
    ...
    async def create(self, user: User) -> User:
        try:
            user_dict = to_mongo_dict(user.model_dump())
            if user_dict.get("birthday"):
                user_dict["birthday"] = datetime.combine(
                    user_dict["birthday"], datetime.min.time()
                )
    ...
    async def update_user_by_id(self, user_id: UUID, updates: UserUpdate) -> User:
        update_data = updates.model_dump(exclude_unset=True)

        if "birthday" in update_data and update_data["birthday"]:
            update_data["age"] = calculate_age(update_data["birthday"])
        updated_user = await self.partial_update(set=update_data, user_id=user_id)
        if not updated_user:
            raise UserNotFoundException(f"User {user_id} not found in database")
        return updated_user
    ...
    async def partial_update(
        ...
        **query: Any,
        ...
        try:
            ...
            if set is not None:
                normalized_set = await self.normalize(set)
                update_operations["$set"] = to_mongo_dict(normalized_set)

            if push is not None:
                normalized_push = await self.normalize(push)
                update_operations["$push"] = to_mongo_dict(normalized_push)
            query = await self.normalize(query)
