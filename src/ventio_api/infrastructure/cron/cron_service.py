...
class CronService:
	...
	async def summarize_stale_conversations(self):
		...
		for convo in conversations:
			try:
				messages = await self.messages_db.get_many(conversation_id=convo.conversation_id)
				## mock llm call
				summary = await llm.summarize("\n".join([message.message for message in messages]))

				await self.summaries_db.insert(Summary(
					summary_id=uuid4(),
				...
				print(f"[CRON] Failed to process conversation {convo.conversation_id}: {e}")

			await self.delete_exceeding_summaries(convo.conversation_id)


	async def delete_exceeding_summaries(self, convo_id):
		summaries = await self.summaries_db.get_many(conversation_id=convo_id, sort=[("timestamp", -1)])
		if len(summaries) > 3:
